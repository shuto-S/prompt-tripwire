...
The official rules take precedence if they change. This matrix records the requirement text as checked against the judge-facing repository and v0.1.3 patch release candidate. The public v0.1.2 release and its demo capture remain historical evidence.
...
| The project installs and runs consistently on the declared platform. | v0.1.3 macOS arm64 candidate, pinned Node/Codex requirements, transactional install/upgrade, owned uninstall boundaries, artifact smoke script, and two-build reproducibility gate. The patch recognizes only the pinned App Server's exact zsh command envelopes, isolates `ZDOTDIR`, and fails closed for missing raw commands, failed unsafe items, and direct `.git` metadata reads. | Pending final v0.1.3 verification/publication |
...
| Developer tools need installation instructions, supported platforms, and a way to test without rebuilding. | `docs/JUDGE_GUIDE.md`; v0.1.3 compiled JavaScript/runtime candidate path; safe local fixture; recorded read-only replay. | Pending final v0.1.3 artifact verification |
| Provide free, unrestricted working-project access through the judging period. | The public repository and v0.1.2 release are reachable without authentication and were anonymously verified. The v0.1.3 patch distribution still needs GitHub publication and anonymous verification. No hosted account or paid PromptTripwire service is required; Codex usage remains under the judge's own OpenAI access. | Pending v0.1.3 publication |
...
1. Run the complete v0.1.3 source, reproducibility, and release-artifact verification gates.
2. Publish the v0.1.3 archive/checksum and verify the repository, release, and judge instructions anonymously.
...
- public release: [v0.1.2](https://github.com/shuto-S/prompt-tripwire/releases/tag/v0.1.2); v0.1.1 and v0.1.0 remain preserved as earlier evidence
...
v0.1.3 compatibility patch. The v0.1.3 GitHub Release URL, archive digest,
anonymous-download check, and public YouTube URL remain pending. No v0.1.1 or
v0.1.2 checksum verifies the v0.1.3 archive.
