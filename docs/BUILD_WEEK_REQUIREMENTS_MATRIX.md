...
The official rules take precedence if they change. This matrix records the requirement text as checked against the judge-facing repository and prepared v0.1.8 release. The public v0.1.2 demo capture and v0.1.3 through v0.1.7 releases remain historical evidence.
...
| The project installs and runs consistently on the declared platform. | Prepared v0.1.8 macOS arm64 artifact, pinned Node/Codex requirements, transactional install/upgrade, owned uninstall boundaries, artifact smoke script, and two-build reproducibility gate. It preserves v0.1.7 policy precision and earlier Plugin/UI boundaries while constraining plan commands to literal argv strings. Public-asset and real Plugin verification are release gates. | Candidate verification pending |
...
| Developer tools need installation instructions, supported platforms, and a way to test without rebuilding. | `docs/JUDGE_GUIDE.md`; prepared v0.1.8 compiled JavaScript/runtime artifact; safe local fixture; recorded read-only replay. | Candidate verification pending |
| Provide free, unrestricted working-project access through the judging period. | The public repository and v0.1.2 through v0.1.7 releases are reachable without authentication and were anonymously verified. v0.1.8 publication remains pending. No hosted account or paid PromptTripwire service is required; Codex usage remains under the judge's own OpenAI access. | Continue monitoring; v0.1.8 pending |
...
v0.1.8 judge distribution. v0.1.8 publication, archive digest, packaged
...
remains pending. The v0.1.7 archive and checksum remain immutable historical
evidence and do not verify v0.1.8.
